...
var websocketclient = {
    ...
    'connect': function () {
        var ops = getOptionsFromConnectionString($('#connectionString').val());
        var host = ops.host;
        $('#urlInput').val(host);
        var port = ops.port;
        $('#portInput').val(port)
        var clientId = ops.clientId;
    ...
    'onConnect': function () {
        ...
        $('#publishTopic').val('devices/' + clientId + '/messages/events/');
        websocketclient.subscribe('devices/' + clientId + '/messages/devicebound/#',parseInt($('#QoSInput').val()),$('#colorChooser').val().substring(1));
